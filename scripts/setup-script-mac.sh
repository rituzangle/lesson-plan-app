...
# NOTE: Supabase has updated API key terminology
# Use either the legacy format OR the new format (not both)

# Legacy format (still supported)
EXPO_PUBLIC_SUPABASE_URL=your_supabase_url_here
EXPO_PUBLIC_SUPABASE_ANON_KEY=your_supabase_anon_key_here

# New format (recommended for new projects)
# EXPO_PUBLIC_SUPABASE_URL=your_supabase_url_here  
# EXPO_PUBLIC_SUPABASE_PUBLISHABLE_KEY=your_supabase_publishable_key_here

# For backend operations (if needed)
# SUPABASE_SERVICE_ROLE_KEY=your_service_role_key_here  # Legacy
# SUPABASE_SECRET_KEY=your_secret_key_here              # New format
...
EOF
    print_status "Created .env template with updated Supabase key formats"
    print_warning "Remember to fill in your Supabase credentials in .env"
    print_warning "Use either legacy (anon) OR new (publishable) key format - not both"
fi
